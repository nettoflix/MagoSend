    ...
    ui(new Ui::MainWindow)
{
    ...
    //Signals e slots referente ao carregamento de arquivos na lista (QTableWidget)
    worker2 = new Worker(this);
    workerThread2.start();
    worker2->moveToThread(&workerThread2);
    connect(this, &MainWindow::triggerWorker, worker2 ,[this]()
    {
        worker2->getFilesAlreadyPresentOnHosts(this->filePaths);
    });
    connect(worker2, &Worker::filesAlreadyPresentOnHostsResult, this, &MainWindow::pupulateGuiTable);
    connect(this, &MainWindow::populateQueueWithPaths, worker2, &Worker::onPopulateQueueWithPaths, Qt::QueuedConnection);
    connect(worker2, &Worker::queueDonePopulating, this, &MainWindow::onQueueDonePopulating, Qt::QueuedConnection);
    connect(worker2, &Worker::idAlreadyExistOnHost, this, &MainWindow::updateQueueItemInformation);
    connect(this, &MainWindow::markHostAsRemovedFromTable, worker2, &Worker::onMarkHostAsRemovedFromTable, Qt::QueuedConnection);
    connect(this, &MainWindow::registerIp, worker2, &Worker::onRegisterIp, Qt::QueuedConnection);
    //connect(&timer_loadTable, &QTimer::timeout, this, &MainWindow::addQueueToTableWidget);



    //thread que verifica se o host esta online
    ...
    connect(worker, &Worker::hostTableTextColorChanged, this, &MainWindow::changeHostsTableTextColor, Qt::QueuedConnection);
    ...
    if(ans == QMessageBox::Ok)
    {
        ...
        if(currentRow != -1)
        {
            ...
            else
            {
                ...
                worker2->checkIfIdExistsOnHosts(ip, newId);
...
void MainWindow::updateModComboBox()
{
    ...
    qDebug(" MainWindow::updateModComboBox() -1");
    qDebug() << " MainWindow::updateModComboBox - Main Thread:" << QThread::currentThread();
    emit getModalidadesFromHosts(items);
    qDebug(" MainWindow::updateModComboBox() -OUT");
}
...
void MainWindow::onTimer()
{
    ...
    if(worker2->isPopulatingQueue) return; //se a workerThread estiver populando a fila, não podemos tentar pegar o lock da queueLocker, se não a GUI thread vai freezar
...
{
    qDebug("on_btn_connectSession_clicked -1");
    ...
    //    qDebug("on_btn_connectSession_clicked: rowCount [%d]", ui->hostsTable->rowCount());
    int rowCount = ui->hostsTable->rowCount();
    for(int i=rowCount-1; i>=0; i--)
    {
        qDebug("ip [%s]", ui->hostsTable->item(i,1)->text().toLatin1().data());
        emit markHostAsRemovedFromTable(ui->hostsTable->item(i,1)->text());
        //hostControl->markHostAsRemovedFromTable(ui->hostsTable->item(i,1)->text());
        ui->hostsTable->removeRow(i);
        //qDebug("remove i [%d]", i);
    }
    ui->hostsTable->setRowCount(0);
    qDebug("on_btn_connectSession_clicked -2");
    ...
    qDebug("on_btn_connectSession_clicked -3");
    ...
    qDebug("on_btn_connectSession_clicked -4");
    updateModComboBox();
}
...
Worker *MainWindow::getWorker2() const
{
    return worker2;
}
...
void MainWindow::pupulateGuiTable(QList<CFilesOnHost> listFilesOnHost)
{
    ...
    if(willPopulateQueue == false) //se tiver algum item pra colocar na lista, o populateQueueWithPaths vai cuidar de parar o spinner quando terminar. Mas no caso de nenhum video ser adicionado, temos que parar o spinner agora
    {
        stopSpinner();
        ui->btn_pausar->setEnabled(true);
        ui->btn_Remover->setEnabled(true);
...
QString MainWindow::getVideoDuration(QString filePath) {
    ...
    QProcess process;
#ifdef __WIN32
    QString command = "ffprobe.exe";
    QStringList arguments = {"-v", "error", "-select_streams", "v:0", "-show_entries", "format=duration", "-of", "default=noprint_wrappers=1:nokey=1", filePath};
#else
    QString command = "ffprobe";
    QStringList arguments = {"-v", "error", "-select_streams", "v:0", "-show_entries", "format=duration", "-of", "default=noprint_wrappers=1:nokey=1", filePath.toLatin1().data()};
#endif
    ...
    if (process.waitForFinished(15000))
    {
        ...
        int duration = output.toFloat();
        int hours = duration / 3600;         // 1 hora = 3600 segundos
        int minutes = (duration % 3600) / 60; // 1 minuto = 60 segundos
        int seconds = duration % 60;  // o resto sao os segundos
        ...
        if(hours > 0)
        {
            sduration += QString::number(hours) + "h ";
        }
        if(minutes > 0)
        {
            sduration += QString::number(minutes) + "m ";
        }
        if(seconds >0)
        {
            sduration	+= QString::number(seconds) + "s";
        }
        //qDebug("sduration [%s]", sduration.toLatin1().data());
        return sduration;
    }
    else
    {
